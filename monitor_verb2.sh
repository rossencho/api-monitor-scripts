...
API_LINK="http://localhost:8000/products"
CURL_BULK_GET="curl http://localhost:8000/products -H $CT "
CURL_GET="curl http://localhost:8000/products/$RAND_NUMBER -H $CT -s -o /dev/null -w %{time_starttransfer}"
CURL_DELETE="curl -X DELETE http://localhost:8000/products/$RAND_NUMBER -H $CT -s -o /dev/null -w %{time_starttransfer}"
POST_PAYLOAD="
{
    \"name\":\"Rossens API test\",
    \"description\":\"Rossens API test from bash\",
	\"price\":1.232,
	\"comment\":\"Rossens API test from bash\"
}"
POST_PAYLOAD_4_JAVA="
{
...
UPDATE_PAYLOAD="
{
	...
	\"price\":1.232,
    \"comment\":\"Rossens API test update\"
}"

CURL_POST="curl http://localhost:8000/products -H $CT -d $POST_PAYLOAD"
GET_VERB="GET"
...
start_server() {  
    `nohup php -S localhost:8000 -t ../slimDoctrine/public >/dev/null &`
    PID=$(pidof "$1")
    echo "Process ID=$PID"
...
pinger() {
    ...
    while [ $status -ne 200 ] ; do
        status=`curl $1 -o /dev/null -w '%{http_code}' -s` 
        logger 0 LAUNCHING
    done
    # average+per_verb php POST
